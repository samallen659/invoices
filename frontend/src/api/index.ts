import { Invoice, InvoiceItem, InvoiceRes, Item, LoginRequest } from "../types";
...
async function getAllInvoices(): Promise<Invoice[]> {
  ...
  const invoices: Invoice[] = invReqs.map((iv: InvoiceRes) =>
    invoiceResToInvoice(iv),
  );
  return invoices;
}
...
async function loginUser(l: LoginRequest) {
...
export { getAllInvoices, deleteInvoice, newInvoice, editInvoice, loginUser };
